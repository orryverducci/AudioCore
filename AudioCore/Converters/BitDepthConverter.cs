...
namespace AudioCore.Converters
{
    ...
    public static class BitDepthConverter
    {
        ...
        public static byte[] ToPCM(double[] samples, byte outputBitDepth, bool floatingPoint = false)
        {
        ...
        public static double[] FromPCM(byte[] bytes, byte bitDepth, bool floatingPoint = false)
        {
        ...
        public static byte[] To8Bit(double[] samples, bool signed = false)
        {
        ...
        public static byte[] To16Bit(double[] samples, bool signed = true)
        {
        ...
        /// <param name="samples">The samples to convert.</param>
        /// <param name="signed">If the PCM audio data should be signed. This parameter is optional and set to <c>true</c> by default.</param>
        public static byte[] To24Bit(double[] samples, bool signed = true)
        {
        ...
        /// <param name="samples">The samples to convert.</param>
        /// <param name="signed">If the PCM audio data should be signed. This parameter is optional and set to <c>true</c> by default.</param>
        public static byte[] To32Bit(double[] samples, bool signed = true)
        {
        ...
        /// <param name="samples">The samples to convert.</param>
        public static byte[] ToFloat(double[] samples)
        {
        ...
        /// As samples in AudioCore are already 64 bit floating-point PCM, this is a convenience method which returns the samples as bytes.
        /// </summary>
        /// <returns>Bytes containing 64 bit floating-point PCM audio data.</returns>
        /// <param name="samples">The samples to convert.</param>
        public static byte[] ToDouble(double[] samples)
        {
            // Return samples as bytes
            byte[] convertedSamples = new byte[samples.Length * 8];
        ...
        public static double[] From8Bit(byte[] bytes, bool signed = false)
        {
            // Create an array of doubles big enough to hold all the samples
            double[] convertedSamples = new double[bytes.Length];
            // Convert each byte to a floating-point sample
            for (int i = 0; i < convertedSamples.Length; i++)
            {
                if (!signed)
                {
                    convertedSamples[i] = (bytes[i] / 127.5) - 1;
                }
                else
                {
                    convertedSamples[i] = ((sbyte)bytes[i] / 127d);
                }
            }
            // Return converted samples
            return convertedSamples;
        ...
        public static double[] From16Bit(byte[] bytes, bool signed = true)
        {
            // Create an array of doubles big enough to hold all the samples
            double[] convertedSamples = new double[bytes.Length / 2];
            // Convert each block of bytes to a floating-point sample
            for (int i = 0; i < convertedSamples.Length; i++)
            {
                if (signed)
                {
                    convertedSamples[i] = BitConverter.ToInt16(bytes, i * 2) / 32767d;
                }
                else
                {
                    convertedSamples[i] = (BitConverter.ToUInt16(bytes, i * 2) / 32767.5) - 1;
                }
            }
            // Return converted samples
            return convertedSamples;
        ...
        /// <param name="signed">If the PCM audio data is signed. This parameter is optional and set to <c>true</c> by default.</param>
        public static double[] From24Bit(byte[] bytes, bool signed = true)
        {
            // Create an array of doubles big enough to hold all the samples
            double[] convertedSamples = new double[bytes.Length / 3];
            ...
            for (int i = 0; i < convertedSamples.Length; i++)
            {
                ...
                if (signed)
                {
                    convertedSamples[i] = BitConverter.ToInt32(sampleBytes, 0) / 8388607d;
                }
                else
                {
                    convertedSamples[i] = (BitConverter.ToInt32(sampleBytes, 0) / 8388607.5) - 1;
                }
            }
            // Return converted samples
            return convertedSamples;
        ...
        /// <param name="signed">If the PCM audio data is signed. This parameter is optional and set to <c>true</c> by default.</param>
        public static double[] From32Bit(byte[] bytes, bool signed = true)
        {
            // Create an array of doubles big enough to hold all the samples
            double[] convertedSamples = new double[bytes.Length / 4];
            // Convert each block of bytes to a floating-point sample
            for (int i = 0; i < convertedSamples.Length; i++)
            {
                if (signed)
                {
                    convertedSamples[i] = BitConverter.ToInt32(bytes, i * 4) / 2147483647d;
                }
                else
                {
                    convertedSamples[i] = (BitConverter.ToUInt32(bytes, i * 4) / 2147483647.5) - 1;
                }
            }
            // Return converted samples
            return convertedSamples;
        ...
        public static double[] FromFloat(byte[] bytes)
        {
            // Create an array of doubles big enough to hold all the samples
            double[] convertedSamples = new double[bytes.Length / 4];
            // Convert each block of bytes to a 32 bit floating-point sample
            for (int i = 0; i < convertedSamples.Length; i++)
            {
                convertedSamples[i] = BitConverter.ToSingle(bytes, i * 4);
            }
            // Return converted samples
            return convertedSamples;
        }
        ...
        /// As samples in AudioCore are already 64 bit floating-point PCM, this is a convenience method which returns the bytes as samples.
        /// </summary>
        /// <returns>The converted audio samples.</returns>
        /// <param name="bytes">Bytes containing 64 bit floating-point PCM audio data.</param>
        public static double[] FromDouble(byte[] bytes)
        {
            // Return bytes as samples
            double[] convertedSamples = new double[bytes.Length / 8];
